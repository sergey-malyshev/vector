...
/**
  ...
  */
uint32_t getSysClkFreq(RCU_SysClk_TypeDef Clk)
{
    uint32_t clk_freq = 0;
...
/**
  ...
  * @retval  Val  Значение Гц
  */
uint32_t getPeriphClkFreq(RCU_PeriphClk_TypeDef Clk)
{
    uint32_t clk_freq = 0;
...
/**
  ...
  * @retval  Val  Значение Гц
  */
uint32_t getSysPeriphClkFreq(RCU_SysPeriphClk_TypeDef Clk)
{
    uint32_t clk_freq = 0;
    uint32_t div = (1 << RCU->APBCFG_bit.DIV);
...
OperationStatus RCU_PLL_AutoConfig(uint32_t SysClkFreq, RCU_PLL_Ref_TypeDef Ref)
{
    ...
    /* Дополнительные циклы ожидания для MFLASH и BFLASH */
    ...
    // Очистка и включение кэша (инструкции + данные) для MFLASH
